...
4. **Given** 화면 조합 입력이 완전할 때, **When** 사용자가 생성 요약을 승인하고 프로젝트를 생성·검증하면, **Then** 별도 View target과 승인된 표준 화면 요소 조합이 생성되고 요구사항별 결과가 제공된다.
...
3. **Given** 직접 구성 영역을 포함한 생성 요약이 승인되었을 때, **When** 프로젝트를 생성하고 검증하면, **Then** 승인된 고유 동작만 생성되고 표준 영역과의 책임 경계가 요구사항에 추적된다.
...
- **FR-017**: 시스템은 사용자가 생성 요약을 명시적으로 승인한 후에만 프로젝트를 생성해야 한다.
